...
/*如果你使用DMA，那么你需要一个“framebuffer”来存储要显示的数据。

*如果您的MCU没有足够的RAM，请避免使用DMA(或设置5到1)。

如果您的MCU有足够的RAM(甚至大于全帧大小)，

*然后你可以指定帧缓冲区大小到完整的分辨率如下。
*/
#define HOR_LEN 	5	//	还要注意屏幕的分辨率!
uint16_t disp_buf[ST7789_WIDTH * HOR_LEN];
#endif

/**
 * @brief 写入命令到ST7789控制器
 * @param  CMD ->命令写入
 * @return none
 */
...
/**
 * @brief Write data to ST7789 controller写入数据到ST7789控制器
 * @param buff -> pointer of data buffer 数据缓冲区指针
 * @param buff_size -> size of the data buffer数据缓冲区的大小
 * @return none
 */
...
/**
 * @brief Write data to ST7789 controller, simplify for 8bit data.写入数据到ST7789控制器，简化为8位数据
 * data -> data to write
 * @return none
 */
...
void ST7789_Init(void)
{
    ...
    ST7789_WriteCommand(ST7789_COLMOD);		//	Set color mode 设置颜色模式
